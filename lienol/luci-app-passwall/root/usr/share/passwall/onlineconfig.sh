...
add_servers(){
	...
	[ -z "$1" ] && config_t_set servers group $ssindex $group
...
add_ss_servers(){
	...
	config_t_set servers local_port $ssindex 1080
	config_t_set servers fast_open $ssindex false
	config_t_set servers server_type $ssindex ss
	uci commit shadowsocks
}
...
del_all_config(){
	...
	is_sub_server=`uci -q get shadowsocks.$current_global_server.group`
	for i in `seq $ssindex -1 1`
	do
...
update_global_server(){
	...
	if [ "$current_global_server" != "nil" ]; then
		[ -f "/var/etc/shadowsocks.json" ] && current_server=$(cat /var/etc/shadowsocks.json| grep "_comment\""| awk -F'"' '{print $4}') || exit 0
...
add() {
	SSR_LINKS=$(cat /usr/share/shadowsocks/ssr_link.conf 2>/dev/null)
	[ -n "$SSR_LINKS" ] && {
		[ -f "$LOCK_FILE" ] && return 3
		touch "$LOCK_FILE"
		mkdir -p /usr/share/shadowsocks/serverconfig
		...
		do
			if [ -n "`echo -n "$ssrlink" | grep "ssr://"`" ]; then
				...
				is_added=$(uci show shadowsocks | grep -v "group=" | grep -c "server='$server'")
		...
		[ -f "/usr/share/shadowsocks/serverconfig/all_onlineservers" ] && rm -f /usr/share/shadowsocks/serverconfig/all_onlineservers
		rm -f "$LOCK_FILE"
	}
}
